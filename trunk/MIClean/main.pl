...
use lib "$FindBin::Bin/Mod"; #Adds the PasswordGen folder into the libary string so that it can find Session::Token
use lib "$FindBin::Bin/Mod/Session"; #Adds the PasswordGen/Session folder into the library so that Dynaloader can find its way down the tree to the .so.
